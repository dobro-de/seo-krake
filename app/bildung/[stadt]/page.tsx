...
export async function generateMetadata({
  ...
  return {
    ...
    alternates: { canonical: `https://kiberatung-v2.vercel.app/bildung/${city.slug}` },
    openGraph: {
      title: city.metaTitle,
      description: city.metaDescription,
      url: `https://kiberatung-v2.vercel.app/bildung/${city.slug}`,
...
// ─── Page ────────────────────────────────────────────────────────────────────
export default async function BildungStadtPage({
  params,
}: {
  params: Promise<{ stadt: string }>;
}) {
        ...
        <section style={{ background: "var(--bg-card)", padding: "2rem 0", borderTop: "1px solid var(--border-subtle)" }}>
          <div className="container">
            <p style={{ color: "var(--text-gray)", fontSize: "13px", marginBottom: "10px" }}>
              KI Beratung Bildung in weiteren Städten:
            </p>
