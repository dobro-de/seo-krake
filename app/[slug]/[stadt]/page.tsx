...
const CITIES: { slug: string; name: string }[] = [
  ...
  { slug: "trier", name: "Trier" },
];

export function generateStaticParams() {
  const branchenSlugs = getAllBranchenSlugs().filter(
    (s) => !DEDICATED_PAGES.includes(s)
  );
  return branchenSlugs.flatMap((slug) =>
    CITIES.map((city) => ({ slug, stadt: city.slug }))
  );
}
...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  if (!b || DEDICATED_PAGES.includes(slug)) return {};
  const city = CITIES.find((c) => c.slug === stadt);
  if (!city) return {};
  return {
    title: `KI Beratung für ${b.name} in ${city.name} | kiberatung.de`,
    description: `KI-Lösungen für ${b.name}-Unternehmen in ${city.name}: ${b.loesungen
      .slice(0, 2)
      .map((l) => l.title)
      .join(", ")} und mehr. Kostenlose Erstberatung.`,
  };
}

export default async function SlugStadtPage({ params }: Props) {
  const { slug, stadt } = await params;

  // Only handle non-dedicated branches
  if (DEDICATED_PAGES.includes(slug)) notFound();
  ...
  const intro = b.stadtIntros[stadt] ?? `Wir beraten ${b.name}-Unternehmen in ${city.name} — mit praxiserprobten KI-Lösungen und lokalem Know-how.`;
  ...
  return (
    <div style={accentStyle}>
      ...
      <main>
        ...
        <section className="branche-hero" style={{ paddingBottom: "4rem" }}>
          <div className="container">
            <nav className="branche-breadcrumb" aria-label="Breadcrumb">
              ...
              <Link href={`/${slug}`}>{b.name}</Link><span>›</span>
              <span>{city.name}</span>
            </nav>
            ...
            <h1 className="branche-title font-display">
              KI BERATUNG FÜR <span className="text-accent">{b.name.toUpperCase()}</span>{" "}
              IN {city.name.toUpperCase()}
            </h1>
            <p className="branche-subtitle">{intro}</p>
          ...
          <div className="container">
            <div className="branche-section-head">
              ...
              <h2 className="section-heading section-heading-center">
                Was hält {b.name}-Unternehmen in {city.name} zurück?
              </h2>
            </div>
        ...
        <section id="loesungen" className="branche-section">
          <div className="container">
            <div className="branche-section-head">
              ...
              <h2 className="section-heading section-heading-center">
                Was wir für {b.name} in {city.name} tun
              </h2>
            </div>
        ...
        <section className="branche-cta-section">
          <div className="container">
            <div className="branche-cta-inner">
              <h2 className="section-heading">Bereit für KI in Ihrer {b.name} in {city.name}?</h2>
        ...
        <section className="branche-section branche-challenges-bg">
          <div className="container">
            <h3 className="branche-more-label">{b.name} KI-Beratung in weiteren Städten</h3>
            <div className="branche-more-pills">
              {CITIES.filter((c) => c.slug !== stadt).map((c) => (
                <Link key={c.slug} href={`/${slug}/${c.slug}`} className="branche-pill">
                  {c.name}
                </Link>
              ))}
            </div>
