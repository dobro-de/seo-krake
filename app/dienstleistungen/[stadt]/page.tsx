...
export async function generateMetadata({
  ...
  return {
    ...
    alternates: { canonical: `https://kiberatung-v2.vercel.app/dienstleistungen/${city.slug}` },
    openGraph: {
      title: city.metaTitle,
      description: city.metaDescription,
      url: `https://kiberatung-v2.vercel.app/dienstleistungen/${city.slug}`,
...
// ─── Page ────────────────────────────────────────────────────────────────────
export default async function DienstleistungenStadtPage({
  params,
}: {
  params: Promise<{ stadt: string }>;
}) {
        ...
        <section style={{ background: "var(--bg-card)", padding: "2rem 0", borderTop: "1px solid var(--border-subtle)" }}>
          <div className="container">
            <p style={{ color: "var(--text-gray)", fontSize: "13px", marginBottom: "10px" }}>
              KI Beratung Dienstleistungen in weiteren Städten:
            </p>
            <div style={{ display: "flex", flexWrap: "wrap", gap: "8px" }}>
              {relatedCities.map((c) => (
                <Link
                  key={c.slug}
                  href={`/dienstleistungen/${c.slug}`}
